...
//    }
@GetMapping // Searching for products by productName
public ResponseEntity<Page<ProductResponseDTO>> getProducts(
        @RequestParam String searchQuery,
        Pageable pageable) {
    ...
    Page<ProductResponseDTO> products = productsService.getProductsByName(searchQuery, pageable);
    logger.info("Successfully fetched {} products for search query '{}', page number: {}, page size: {}",
            products.getTotalElements(), searchQuery, pageable.getPageNumber(), pageable.getPageSize());
