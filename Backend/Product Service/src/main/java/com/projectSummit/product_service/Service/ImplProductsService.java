...
//    }
    public Page<ProductResponseDTO> getProductsByName(String searchQuery, Pageable pageable) {
        if (searchQuery == null || searchQuery.trim().isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Search query cannot be empty.");
        }

        logger.info("Searching products by name: {}", searchQuery);

        Page<Product> productsPage = productsRepository.findByProdNameContainingIgnoreCase(searchQuery, pageable);
