...
@PatchMapping("/{orderId}")
public ResponseEntity<String> cancelOrder(@PathVariable int orderId, @RequestBody OrderStatusDTO orderStatusDTO) {
    logger.info("Received request to cancel order with ID: {} and status: {}", orderId, orderStatusDTO.status());

    try {
        boolean isOrderExists = ordersService.isOrderExists(orderId);
        if (!isOrderExists) {
            logger.error("Order with ID: {} not found.", orderId);
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Order not found");
        }
        ...
        ordersService.cancelOrder(orderId, orderStatusDTO);
